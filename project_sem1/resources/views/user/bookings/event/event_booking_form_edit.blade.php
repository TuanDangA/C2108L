    ...
    <div style="margin-left:0px;width:96%;margin-top:40px;">
        @if (!is_null($accounts))
            @if (!is_null($old_booking_event))
                {{ Breadcrumbs::render('user_event_booking_edit',$accounts->confirmation_code,$event->href_param,$old_booking_event->id) }}        
            @endif
            @if (is_null($old_booking_event))
                {{ Breadcrumbs::render('user_event_booking_edit',$accounts->confirmation_code,$event->href_param,0) }}        
            @endif
        @endif
        @if (is_null($accounts))
            @if (!is_null($old_booking_event))
            {{ Breadcrumbs::render('user_event_booking_edit',"guest",$event->href_param,$old_booking_event->id) }}        
            @endif
            @if (is_null($old_booking_event))
            {{ Breadcrumbs::render('user_event_booking_edit',"guest",$event->href_param,0) }}        
            @endif  
        @endif
    </div>
        ...
        <form method="post" action="{{ route('user_event_booking_confirm',['confirmation_code'=>$accounts->confirmation_code,'href_param'=>$event->href_param]) }}">    
            ...
            <div class="contact">
                ...
                <input type="hidden" name="id_old_booking_event"
                    @if (isset($request->id_old_booking_event)) value="{{ $request->id_old_booking_event }}" @endif
                    @if (!isset($request->id_old_booking_event)) value="0" @endif
                >
            ...
            <div class="request">
                ...
                <div class="row">
                    ...
                    <textarea class="form-control col-3" name="details" rows="4" id="comment" placeholder="Note....">{{ $request->details }}</textarea>
                </div>
                <div class="row">
