...
<div style="margin-left:0px;width:96%;margin-top:40px;">
    @if (!is_null($accounts))
        @if (!is_null($old_booking_adults) && !is_null($old_booking_children))
            {{ Breadcrumbs::render('user_visit_booking_form',$accounts->confirmation_code,$old_booking_adults->id,$old_booking_children->id) }}        
        @endif
        @if (is_null($old_booking_adults) && !is_null($old_booking_children))
            {{ Breadcrumbs::render('user_visit_booking_form',$accounts->confirmation_code,0,$old_booking_children->id) }}        
        @endif
        @if (!is_null($old_booking_adults) && is_null($old_booking_children))
            {{ Breadcrumbs::render('user_visit_booking_form',$accounts->confirmation_code,$old_booking_adults->id,0) }}        
        @endif
        @if (is_null($old_booking_adults) && is_null($old_booking_children))
            {{ Breadcrumbs::render('user_visit_booking_form',$accounts->confirmation_code,0,0) }}        
        @endif
    @endif
    @if (is_null($accounts))
        @if (!is_null($old_booking_adults) && !is_null($old_booking_children))
            {{ Breadcrumbs::render('user_visit_booking_form',"guest",$old_booking_adults->id,$old_booking_children->id) }}        
        @endif
        @if (is_null($old_booking_adults) && !is_null($old_booking_children))
            {{ Breadcrumbs::render('user_visit_booking_form',"guest",0,$old_booking_children->id) }}        
        @endif
        @if (!is_null($old_booking_adults) && is_null($old_booking_children))
            {{ Breadcrumbs::render('user_visit_booking_form',"guest",$old_booking_adults->id,0) }}        
        @endif
        @if (is_null($old_booking_adults) && is_null($old_booking_children))
            {{ Breadcrumbs::render('user_visit_booking_form',"guest",0,0) }}        
        @endif  
    @endif
</div>
    ...
    <form method="post" action="{{ route('user_visit_booking_confirm',['confirmation_code'=>$accounts->confirmation_code]) }}">    
        ...
        <div class="request">
            ...
            <div class="form-group">
                ...
                <input class="form-control" id="details" name="details" placeholder="Note...." 
            ...
            <div class="row">
                ...
                <input type="number" class=" form-control col-2" name="quantity_adults" id="quantity_adults" @if(!is_null($old_booking_adults)) value="{{ $old_booking_adults->quantity }}"@endif @if (is_null($old_booking_adults)) value="0" @endif  onchange="showTotalPrice()" required>
            </div>
             <div class="row">
                ...
                <input type="number" class="form-control col-2" name="quantity_children" id="quantity_children" 
                    @if(!is_null($old_booking_children)) value="{{ $old_booking_children->quantity }}"@endif 
                    @if(is_null($old_booking_children)) value="0" @endif 
                onchange="showTotalPrice()" required>
            </div>  
            ...
            <div class="row">
                ...
                <input type="hidden" class="col-3" name="total_price_children" id="total_price_children" 
                    @if(!is_null($old_booking_children)) value="{{ $old_booking_children->price }}"@endif 
                >
                <input type="hidden" class="col-3" name="total_price_adults" id="total_price_adults" 
                    @if(!is_null($old_booking_adults)) value="{{ $old_booking_adults->price }}"@endif 
                >
            </div> 
