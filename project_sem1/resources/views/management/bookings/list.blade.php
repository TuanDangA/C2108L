...
@section('header','NEXUS AQUARIUM | BOOKINGS LIST')
...
@section('content')
    <div class="container" style="margin-top: 100px">
        ...
        {{ Breadcrumbs::render('bookings-list',$accounts->confirmation_code) }}        

        <div style="display: flex; flex-direction: row-reverse; align-items: center; width: 100%; height: 100px;">
            <form action="{{ route('bookings-list', $accounts->confirmation_code) }}" method="get" style="position: relative; display: flex; justify-content: flex-end; align-items: center; width: 75%;">
                @csrf
            ...
            <div style="position: relative; width: auto; margin-right: auto;">
                <a href="{{ route('bookings-add', $accounts->confirmation_code) }}">
            ...
            <tbody>
                @foreach ($bookings as $item)
                    <tr>
                        ...
                        <td style="width: 60px">
                            <a href="{{ route('bookings-edit', array('confirmation_code' => $accounts->confirmation_code, 'id_booking'=>$item->id, 'id_event'=>$item->id_event)) }}">
                                <button class="btn btn-warning">Edit</button>
                            </a>
                        </td>
                        <td style="width: 60px">
                            <button class="btn btn-danger" onclick="deleteBooking({{ $item->id }})">Delete</button>
                        </td>
                    </tr>
                @endforeach
            </tbody>
...
@section('js')
    <script>
        ...
        function deleteBooking(id) {
            var option = confirm('Are you sure to delete this booking?')

            if(!option) return

            $.post('{{ route('bookings-delete', $accounts->id) }}', {
                '_token': '{{csrf_token()}}',
                'id': id
            }, function(data) {
                location.reload()
