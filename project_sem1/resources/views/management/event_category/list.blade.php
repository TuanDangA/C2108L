...
@section('header','NEXUS AQUARIUM | EVENT CATEGORIES LIST')
...
@section('content')
    <div class="container" style="margin-top: 100px">
        ...
        {{ Breadcrumbs::render('event_category-list',$accounts->confirmation_code) }}        

        <div style="display: flex; flex-direction: row-reverse; align-items: center; width: 100%; height: 100px;">
            <form action="{{ route('event_category-list', $accounts->confirmation_code) }}" method="get" style="position: relative; display: flex; justify-content: flex-end; align-items: center; width: 75%;">
                @csrf
            ...
            <div style="position: relative; width: auto; margin-right: auto;">
                <a href="{{ route('event_category-add', $accounts->confirmation_code) }}">
                    <button class="btn btn-success">Add Event Category</button>
                </a>
            </div>
        </div>
        <br>
            ...
            <tbody>
                @foreach ($category as $item)
                    <tr>
                        ...
                        <td style="width: 60px">
                            <a href="{{ route('event_category-edit', array('confirmation_code' => $accounts->confirmation_code, 'id'=>$item->id)) }}">
...
@section('js')
    <script>
        ...
        function deleteCategory(id) {
            ...
            $.post('{{ route('event_category-delete', $accounts->id) }}', {
                '_token': '{{csrf_token()}}',
                'id': id
            }, function(data) {
                location.reload()
